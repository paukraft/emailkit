...
const normalizeHtml = (html: string) => html.replace(/​/g, "").trim()

const sanitizeOutgoingHtml = (html: string) => {
  const normalized = normalizeHtml(html)
  if (EMPTY_HTML_MARKERS.has(normalized)) return ""
  return normalized.replace(/\sclass="[^"]*"/g, "").replace(/\sdir="[^"]*"/g, "")
}
...
const readToolbarState = (editor: LexicalEditor): ToolbarState =>
  editor.getEditorState().read(() => {
    ...
    const listNode = $findMatchingParent(
      selectedNode,
      (candidate) => $isElementNode(candidate) && candidate.getType() === "list",
    )
    if (listNode && $isListNode(listNode)) {
      next.list = listNode.getListType() === "number" ? "number" : "bullet"
    }
...
const setEditorHtml = (editor: LexicalEditor, html: string) => {
  editor.update(() => {
    const root = $getRoot()
    root.clear()
...
function ToolbarPlugin() {
  ...
  useEffect(() => editor.registerUpdateListener(() => setToolbar(readToolbarState(editor))), [editor])
  ...
  return (
    <div className="flex items-center gap-0.5 border-b px-1.5 py-1">
      <ToolbarButton icon={Bold} title="Bold" active={toolbar.bold} onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, "bold")} />
      <ToolbarButton icon={Italic} title="Italic" active={toolbar.italic} onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, "italic")} />
      <ToolbarButton icon={Underline} title="Underline" active={toolbar.underline} onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, "underline")} />
      <ToolbarButton icon={Strikethrough} title="Strikethrough" active={toolbar.strikethrough} onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, "strikethrough")} />
      <span className="mx-0.5 h-3 w-px bg-border" />
      <ToolbarButton icon={Heading1} title="Heading 1" active={toolbar.block === "h1"} onClick={() => setBlock(toolbar.block === "h1" ? "paragraph" : "h1")} />
      <ToolbarButton icon={Heading2} title="Heading 2" active={toolbar.block === "h2"} onClick={() => setBlock(toolbar.block === "h2" ? "paragraph" : "h2")} />
      <ToolbarButton icon={Code} title="Code" active={toolbar.code} onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, "code")} />
      <span className="mx-0.5 h-3 w-px bg-border" />
      <ToolbarButton icon={List} title="Bullet list" active={toolbar.list === "bullet"} onClick={() => toggleList("bullet")} />
      <ToolbarButton icon={ListOrdered} title="Numbered list" active={toolbar.list === "number"} onClick={() => toggleList("number")} />
      <span className="mx-0.5 h-3 w-px bg-border" />
      <ToolbarButton icon={LinkIcon} title="Insert link" active={toolbar.link} onClick={toggleLink} />
      <ToolbarButton icon={Unlink} title="Remove link" disabled={!toolbar.link} onClick={() => editor.dispatchCommand(TOGGLE_LINK_COMMAND, null)} />
    </div>
  )
}

function HtmlSyncPlugin({ value, onChange }: { value: string; onChange: (html: string) => void }) {
  const [editor] = useLexicalComposerContext()
          ...
          <RichTextPlugin
            contentEditable={
              <ContentEditable
                ...
                placeholder={<></>}
                className="scrollbar-thin max-h-[300px] min-h-[120px] overflow-y-auto px-2.5 py-2 text-xs leading-relaxed text-foreground outline-none"
              />
            }
            placeholder={
              <div className="pointer-events-none absolute top-2 left-2.5 text-xs text-muted-foreground">
                Write your message...
              </div>
            }
