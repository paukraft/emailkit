...
:::note
`context` travels as JSON in the mailbox's AIInbx metadata — 500 characters, so pass ids, not a large object — and comes back on `onConnected` and `onDeleted`. `ref` and your own `metadata` stay free to set through `provider`. Mailboxes connected through AIInbx's hosted page or connect link arrive with their plain `ref` string as `context`. A connect URL is single-use once the customer consents. `onDeleted` can fire twice for a mailbox you delete yourself — once from the call, once from the webhook — so keep it idempotent.
:::
