...
:::note
AIInbx delivery events identify an email by its AIInbx id, so `sendEmail` returns that id (`eml_…`) as `messageId` — store it and match events on `event.messageId`. Inbound `messageId` stays the RFC Message-ID.
:::
