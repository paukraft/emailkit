...
:::tip
Don't want to run OAuth apps, token storage, and subscriptions yourself? [AIInbx](/providers/aiinbx#connect-gmail-and-outlook-inboxes) connects Gmail and Outlook inboxes through a hosted flow and keeps the tokens — same `mailboxes.connect()` call, nothing to store.
:::
